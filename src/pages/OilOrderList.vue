...
<script>
  ...
  export default {
    ...
    computed: {
      objData () {
        if (localStorage.getItem('OilOrderList')) {
          let temp = JSON.parse(localStorage.getItem('OilOrderList'))
          if (this.$route.query.data.name) {
            temp.unshift(this.$route.query.data)
            localStorage.setItem('OilOrderList', JSON.stringify(temp))
          }
        } else {
          if (this.$route.query.data.name) {
            let temp = []
        ...
        let listArr = JSON.parse(localStorage.getItem('OilOrderList'))
        return listArr
      }
    }
  }
