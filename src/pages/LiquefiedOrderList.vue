...
<script>
  ...
  export default {
    ...
    computed: {
      objData () {
        if (localStorage.getItem('LiquefiedOrderList')) {
          let temp = JSON.parse(localStorage.getItem('LiquefiedOrderList'))
          if (this.$route.query.data.name) {
            temp.unshift(this.$route.query.data)
            localStorage.setItem('LiquefiedOrderList', JSON.stringify(temp))
          }
        } else {
          if (this.$route.query.data.name) {
            let temp = []
        ...
        let listArr = JSON.parse(localStorage.getItem('LiquefiedOrderList'))
        return listArr
      }
    }
  }
