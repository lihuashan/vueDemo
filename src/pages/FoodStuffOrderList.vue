...
<script>
  ...
  export default {
    ...
    computed: {
      objData () {
        if (localStorage.getItem('FoodrderList')) {
          let temp = JSON.parse(localStorage.getItem('FoodrderList'))
          if (this.$route.query.data.name) {
            temp.unshift(this.$route.query.data)
            localStorage.setItem('FoodrderList', JSON.stringify(temp))
          }
        } else {
          if (this.$route.query.data.name) {
            let temp = []
        ...
        let listArr = JSON.parse(localStorage.getItem('FoodrderList'))
        return listArr
      }
    }
  }
