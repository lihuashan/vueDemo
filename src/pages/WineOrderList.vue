...
<script>
  ...
  export default {
    ...
    computed: {
      objData () {
        if (localStorage.getItem('WineOrderList')) {
          let temp = JSON.parse(localStorage.getItem('WineOrderList'))
          if (this.$route.query.data.name) {
            temp.unshift(this.$route.query.data)
            localStorage.setItem('WineOrderList', JSON.stringify(temp))
          }
        } else {
          if (this.$route.query.data.name) {
            let temp = []
        ...
        let listArr = JSON.parse(localStorage.getItem('WineOrderList'))
        return listArr
      }
    }
  }
